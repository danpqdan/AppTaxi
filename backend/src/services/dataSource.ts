...
export const dataSource = new DataSource({
  ...
  dropSchema: true,  // Evita que o esquema do banco seja apagado
...
export const startServer = async () => {
  ...
  try {
    ...
    await queryRunner.query(`
      INSERT INTO drivers (id, name, description, car, tax, km_lowest, value) VALUES
      (1, 'Homer Simpson', 'Olá! Sou o Homer, seu motorista camarada! Relaxe e aproveite o passeio, com direito a rosquinhas e boas risadas (e talvez alguns desvios).', 'Plymouth Valiant 1973 rosa e enferrujado', 2.50, 1, 0),
      (2, 'Dominic Toretto', 'Ei, aqui é o Dom. Pode entrar, vou te levar com segurança e rapidez ao seu destino. Só não mexa no rádio, a playlist é sagrada.', 'Dodge Charger R/T 1970 modificado', 5.00, 5, 0),
      (3, 'James Bond', 'Boa noite, sou James Bond. À seu dispor para um passeio suave e discreto. Aperte o cinto e aproveite a viagem.', 'Aston Martin DB5 clássico', 10.00, 10, 0)
      ON DUPLICATE KEY UPDATE
          name = VALUES(name),
          description = VALUES(description),
          car = VALUES(car),
          tax = VALUES(tax),
          km_lowest = VALUES(km_lowest),
          value = VALUES(value);
  `).then(() => console.log('Drivers query executed successfully.'))
    ...
    await queryRunner.query(`
      INSERT INTO reviews (rating, comment, driver_id) VALUES
      (1, 'Motorista simpático, mas errou o caminho 3 vezes. O carro cheira a donuts.', 1),
      (2, 'Que viagem incrível! O carro é um show à parte e o motorista, apesar de ter uma cara de poucos amigos, foi super gente boa. Recomendo!', 2),
      (3, 'Serviço impecável! O motorista é a própria definição de classe e o carro é simplesmente magnífico. Uma experiência digna de um agente secreto.', 3)
      ON DUPLICATE KEY UPDATE
          rating = VALUES(rating),
          comment = VALUES(comment),
          driver_id = VALUES(driver_id);
  `).then(() => console.log('Reviews query executed successfully.'))
      .catch(error => console.error('Error executing reviews query:', error));
